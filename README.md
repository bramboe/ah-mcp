...
| `ah_get_bonus_offers` | All current bonus/promotional offers with the effective `bonus_price`. Tiered/stapel deals (e.g. "1 stuk 30% / 2 stuks 50%") include a `tiers` array with the price per step. Optional keyword filter. |
...
| `ah_get_bonus_offers_by_type` | Offers of a specific type: `PREMIUM` (AH Premium deals), `AHONLINE`, `ETOS`, `GALL`, `GALLCARD`. Supports `period=next`. |
...
| `ah_get_savings_summary` | Total discount over a period (default: this month) from in-store receipts **and online orders**: split by bonus/premium/miles/koopzegels/online, top discounted products, koopzegels bought, per-receipt and per-order breakdown. Set `include_online=false` for in-store only. |
